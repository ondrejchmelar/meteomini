...
#define ADC_BATT_PIN 0                       // Analog input for battery measurement
#define BATT_DIVIDER_RATIO 1.82274           // Voltage divider ratio for battery measurement
...
void setup() {
  ...
  wifi_manager.autoConnect("ESP32-Config");
  ...
  uint16_t error = sgp41.executeSelfTest(test_result);
  if (error || test_result != 0xD400) {
  ...
  float bat_voltage = analogReadMilliVolts(ADC_BATT_PIN) * BATT_DIVIDER_RATIO / 1000.0;
